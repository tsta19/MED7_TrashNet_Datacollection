...
if __name__ == '__main__':
    ...
    calibrating = True
    check = True
    cap = cv2.VideoCapture('data/outside_videos/mergedCity.mp4')
    ret, frame = cap.read()
    motion = 0
    sparseOF = SparseOF()
    leftCnts = []
    ...
    while cap.isOpened() and calibrating:
        ...
        if frameCount > 150:
            ...
            if frameCount > 500:
                ...
                if len(dfRight) != 0 and max(dfRight['area']) > 300:
                    closing = True
                    closeCounter += 1
                    print(closeCounter)
                ...
                if closing:
                    ...
                    if closeTimer > 10 and closeCounter > 3 and stillClosedBool == False and len(fiftyFrame) > 499:
                        closing = False
                        closeCounter = 0
                        closeTimer = 0
                        screenVal = 250 / 2
                        ...
                        cv2.imwrite("data/savedimagesCity/garbage" + str(imNum) + ".png", resize)
                        # The code line for the neural network segmentation of the image.
                        results, output = ins.segmentImage("data/savedimagesCity/garbage" + str(imNum) + ".png",
                                                           show_bboxes=True,
                                                           output_image_name="data/savedimagesCity/segmented" + str(
                                                               imNum) + ".png")
                        ...
                        if len(results['boxes']) != 0:
                            ...
                            cv2.imwrite("data/savedimagesCity/bbox" + str(imNum) + ".png", resize)
                            f1 = open("data/yoloCity/" + str(imNum) + ".txt", "w+")
